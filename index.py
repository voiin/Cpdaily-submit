...
def sleepCheckFrom(session, host):
    ...
    return {'signInstanceWid': signInstanceWid, 'signWid': signWid, 'longitude': longitude, 'latitude': latitude, 'rateTaskBeginTime': rateTaskBeginTime}
...
#查寝拍照启动函数
def photoMain(user, session, host, nowTime):
    log('正在查询最新待拍照的查寝表单。。。')
    params = sleepCheckFrom(session, host)
    if str(params) != 'None':
        log('查询最新待拍照的查寝表单成功。。。')
        if nowTime < params['rateTaskBeginTime']:
            log('当前未到查寝签到时间。。。')
            exit(-1)
        log('正在自动上传图片至阿里云oss。。。')
    ...
    try:
        for user in config['users']:
            ...
            if session != 'None':
                ...
                if int(nowTime.split(':')[0]) not in range(18, 24):
                    msg = formMain(user, session, apis['host'])
                else:
                    msg = photoMain(user, session, apis['host'],nowTime)
                if msg == 'SUCCESS':
                    log('自动提交成功！')
...
def sendMessage(send,msg):
    if send != '':
        log('正在发送邮件通知。。。')
        headers = {'cookie': 'c7ac858b9240e3cf015e11fbdf9487e6__void_post_views=70%252C8%252C5; _pk_id.1.0124=aa89a0952a18ce13.1600233358.4.1600319507.1600317158.; PHPSESSID=j29jaead4rd0kbo5rdistf0mpu'}
        res = requests.post('https://voiin.com/sendmail', headers=headers, data=json.dumps({'title': '今日校园疫情上报自动提交结果通知', 'content': getTimeStr() + str(msg), 'to': send}))
        res.encoding = 'utf-8'
        code = res.json()['code']
        if code == 0:
           log('发送邮件通知成功。。。')
