...
def getCookies():
    ...
    brower.get(url)
    brower.find_element_by_id('username').send_keys(config['user']['username'])
    brower.find_element_by_id('password').send_keys(config['user']['password'])
    ...
    for line in brower.get_cookies():
        cookies.set(line['name'], line['value'])
...
def main():
    try:
        ...
        if str(cookies) != 'None':
            ...
            if msg == 'SUCCESS':
                log('自动提交成功！')
                sendMessage(config['user']['email'], '自动提交成功！')
                ...
                log('自动提交失败。。。')
                log('错误是' + msg)
                sendMessage(config['user']['email'], '自动提交失败！错误是' + msg)
                exit(-1)
        else:
            log('模拟登陆失败。。。')
